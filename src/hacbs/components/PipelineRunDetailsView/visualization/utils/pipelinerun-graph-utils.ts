...
import { PipelineRunNode, PipelineRunNodeData, PipelineRunNodeType } from '../types';
...
export const taskWhenStatus = (task: any) => {
  if (!task.when) {
    return undefined;
  }
  ...
  dag.topologicalSort((vertex: any) => {
    ...
    nodes.push(
      createPipelineRunNode(PipelineRunNodeType.TASK_NODE, {
        id: vertex.name,
        label: vertex.name,
        width: maxWidthForLevel[vertex.level] + 50,
